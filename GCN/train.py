...
def read_cora(data_root, device):
    ...
    node_feature = dataset.x / dataset.x.sum(1, keepdims=True)  # 对原始不为0的数据进行归一化操作，使得每一行和为1
    # 节点数量 ， 输入维度
...
def main(config, is_draw_loss=False):
    """
    主函数
    Args:
        -------
    ...
    val_acc_history = []
    model.train()
    train_y = y[train_mask]  # shape=（1708，）不是（2708，）了
    # 共进行200次训练
    for epoch in range(config['epochs']):
        logits = model(adjacency, x)  # 前向传播
        train_mask_logits = logits[train_mask]   # 只选择训练节点进行监督 (140,)
        # print(train_mask_logits)
        # print(train_mask_logits.shape)
        # print(train_y)
        train_mask_logits_probs = F.log_softmax(train_mask_logits,dim=1)
        loss = F.nll_loss(train_mask_logits_probs,train_y)
        # loss = criterion(train_mask_logits, train_y)    # 计算损失值  
        # loss = F.nll_loss(train_mask_logits, train_y)
        # loss = F.nll_loss(train_mask_logits, train_y)
...
if __name__=="__main__":
    ...
    # 超参数定义
    DEVICE = "cpu" #设备,使用cpu运行
    config = {
        "lr":0.1,               # 学习率
        "weight_decay":5e-4,    # 权重衰减
        "epochs":200,           # 训练循环次数
        "device": DEVICE,       # 训练设备
        "data_path": "../cora"  # 数据集路径
    }

    main(config=config, is_draw_loss=False)
